...
import qualified Dhall.Html.Parse
    ...
    File hasHome file -> do
        ...
        case pathMode of
            Code -> do
                ...
                case x of
                    Failure errInfo -> do
                        rawContents <- Data.ByteString.readFile string
                        case XH.parseHTML string rawContents of
                          Right (XH.XmlDocument _ _ _) -> fail "parsed document as XML, should not be possible"
                          Right (XH.HtmlDocument _ _ nodes) -> case Dhall.Html.Parse.parseNodes nodes of
                            Left errNodes -> throwIO errNodes
                            Right expr -> return (Annot expr (App List (Var (Dhall.Core.V "Node" 0))))
                          Left _ -> throwIO (ParseError (Text.Trifecta._errDoc errInfo))
                    Success expr -> do
                        return expr
            RawText -> do
